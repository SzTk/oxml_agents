...
async def _build_agent(instructions: str, tools: list) -> StreamingTinyAgent:
...
async def main():
    router_agent = await _build_agent(ROUTER_INSTRUCTIONS, [])
    files_agent = await _build_agent(FILES_INSTRUCTIONS, [scan_current_dir, read_file])
    webpage_agent = await _build_agent(WEBPAGE_INSTRUCTIONS, [visit_webpage])
    search_agent = await _build_agent(SEARCH_INSTRUCTIONS, [search, visit_webpage])

    category_result = await router_agent.run_stream_async(prompt)
    category = category_result.strip().lower()
